...
for (std::size_t i = 0; i < network_data.flow_at_dnodes.size(); ++i) {
    numerator += network_data.flow_at_dnodes[i] * (network_data.head_at_dnodes[i] - network_data.req_head_at_dnodes[i]);
}
...
for (std::size_t i = 0; i < network_data.flow_at_dnodes.size(); ++i) {
    denominator -= network_data.flow_at_dnodes[i] * network_data.req_head_at_dnodes[i];
}
