...
namespace bevarmejo {
	...
	ModelAnytown::ModelAnytown(fsys::path input_directory, pugi::xml_node settings) {
		...
		_anytown_ = std::make_shared<water_distribution_system>();
    ...
    std::vector<double> ModelAnytown::apply_dv(std::shared_ptr<bevarmejo::water_distribution_system> anytown, const std::vector<double> &dv) const
    {
		std::vector<double> old_HW_coeffs;
		// 1. existing pipes
		for (std::size_t i = 0; i < 35; ++i) {
    ...
    void ModelAnytown::reset_dv(std::shared_ptr<bevarmejo::water_distribution_system> anytown, const std::vector<double> &dv, const std::vector<double> &old_HW_coeffs) const {
